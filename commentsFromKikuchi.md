...
# 2023/1/15完成版へのフィードバック（菊池）
## 要旨
- アルファベットのフォントがゴシックになっていますが、意図的でしょうか？そうでないならフォントをそろえた方が良いでしょう。
- Covid-19であっていますか？COVID-19ではないですか？
- 「結果、」という接続詞的用法は口語なので、「その結果として、」のようにきちんと書きましょう。
- 2章で「・・・利用。」のような体言止めがあります。きちんとした文章にしましょう。
- １５分は15分
- 7章が未完成になっていますが、もう時間がないので、現時点で書ける範囲で完成させてください。書けることがないなら全削除でも良いです。紙面が余ったり、恰好がつかなかったりするなら「7. おわりに」として「本アプリケーションSocradoxは・・・で公開予定である。」のように、開発研究としてのゴールを示唆するようなまとめ方をしても良いでしょう。
- 7章の「コミュニティオブプラクティス」が唐突です。[1]のことなら「[1]から作成した」と書くべきだし、そもそもそれは方法のところで述べることなので6章に記すべきでは。
